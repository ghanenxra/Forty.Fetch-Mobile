...
class DownloadEngineService {
  ...
  Future<Stream<String>> startDownload({
    ...
    final ytdlpPath = _binaryManager.getBinaryPath('yt-dlp');
    final ffmpegPath = _binaryManager.getBinaryPath('ffmpeg');
    
    // Validate binaries exist
    if (!await File(ytdlpPath).exists() || !await File(ffmpegPath).exists()) {
      throw Exception('Engine binaries not found. Please restart or update the app.');
    }
    ...
    if (quality == 'MP3') {
      ...
      args.addAll([
        '-f', 'bestvideo[height<=$height][fps<=60]+bestaudio/best[height<=$height][fps<=60]/best',
